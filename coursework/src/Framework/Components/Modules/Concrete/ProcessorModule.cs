...
internal sealed class ProcessorModule : Module
{
    ...
    internal float Busyness => this.currentBusyness / SystemModelController.Instance.IterationsCount;

    internal float QueueMean => this.currentQueueMean / SystemModelController.Instance.IterationsCount;
    ...
    internal override sealed void CompleteTask()
    {
        ...
        SystemLoggerService.Instance.AppendLogEntry(this, "LOG", "TRACE", $"[{base.Identifier}] sends [{finishedTask.Id}] to the [{nextModule?.Identifier}]");
